...
/*	if ($_FILES["file"]["error"] > 0)
	...
	//conteggio parole!!
	$arrayWord = array_count_values(str_word_count($testo, 1));
	$summ = array_sum($arrayWord);
	$vall = $summ/sizeOf($arrayWord);
	//echo $vall;
	
	$trtr = 0;
	foreach ($arrayWord as $key => $value) {
		if(strlen($key) == 1 || $value < $vall){
			unset($arrayWord[$key]);
		} else {
			//echo " - ".$key." - ".$value."<br/>";
		}
	...
	foreach ($arrayWord as $key => $value) {
		if($trtr >= sizeOf($specialChar)){
			...
			$arrayWord[$key] = $specialChar[$trtr]; 
